...
export const zgzl = () =>
  mkRuleClass({
    ...
    author: (() => {
      const authorElement = document.querySelector("#info > p:first-of-type") as HTMLElement;
      if (authorElement) {
        // Clean up the author name
        return authorElement.innerText.replace(/^作\s*者\s*[：:]\s*/u, "").trim();
      }
      return "未知";
    })(),
    introDom: document.querySelector("#intro > p") as HTMLElement,
    introDomPatch: (introDom) => introDom,
    coverUrl: (document.querySelector("#fmimg > img") as HTMLImageElement).src || null,
    aList: (() => {
      // "最新章节" 区域
      ...
      const { contentRaw } = await nextPageParse({
        ...
        contentPatch: (content) => {
          ...
          if (converted.children.length > 1) {
            const lastLine = (converted.lastElementChild as HTMLParagraphElement).innerText;
            const unwantedText = /内容未完.*|本章阅读完毕.*/;
            if (unwantedText.test(lastLine)) {
              converted.removeChild(converted.lastElementChild as HTMLParagraphElement);
            }
          }
        ...
        getNextPage: (doc) => {
          const nextLink = doc.querySelector('div.bottem1 > a:last-of-type') as HTMLAnchorElement;
